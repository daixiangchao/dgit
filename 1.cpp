	...
	: m_Delimiter(delimiter), m_Comment(comment)
{
	...
	if (!ifs) throw File_not_found(filename);
...
int main()
{
	std::string read_file = "1.txt";
	std::string write_file = "Ikun.txt";
	const char ConfigFile[] = "dxc.config";
	Config configSettings(ConfigFile);

	read_file = configSettings.Read("READ_FILE", read_file);
	write_file = configSettings.Read("WRITE_FILE", write_file);
	std::cout << "read_file:" << read_file << std::endl;
	std::cout << "write_file:" << write_file << std::endl;

	std::ifstream ifs;
	std::ofstream ofs;
	ifs.open(read_file, std::ios::in);
	ofs.open(write_file, std::ios::out);
	std::string str;
	while (getline(ifs, str)) ofs << str << std::endl;
	ifs.close();
	ofs.close();
	return 0;
}
